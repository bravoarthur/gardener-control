...
function AddArea({ handlePage }) {
    ...
    const deleteArea = (indexArea) => {
        const indexDelete = clientList.findIndex(
            (item) => item.area === areaList[indexArea]
        );
    ...
    const _handleAddArea = () => {
        const indexNewArea = areaList.findIndex(
            (item) => item === newAreaInput
        );
    ...
    return (
        <DivArea>
            ...
            <ButtonB className="btnBackHome" onClick={() => handlePage(0)}>
                Back Home
            </ButtonB>
        </DivArea>
    );
